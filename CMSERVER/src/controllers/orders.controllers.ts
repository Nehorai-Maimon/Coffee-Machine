...
const create = async (orderData: OrderData) => {
  ...
  await addOrderToQueue(
    ...
    newOrder.title,
    newOrder.delayMinutes,
  );
  return newOrder;
};

const update = async(orderId: string, changes: Object)=> await Order.findByIdAndUpdate(orderId,changes);
...
export { create,update, readAll, readMonthReport, readHistogramData };
